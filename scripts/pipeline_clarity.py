...
def normalize_row(orig_row: dict, clarity_project_id: str, snapshot_date: str, ingested_at_iso: str) -> dict:
    # start with a shallow copy so we don't mutate input
    r = dict(orig_row)
    ...
    # ensure optional dims exist
    for d in ("dimension1","dimension2","dimension3"):
        if d not in r:
            r[d] = None

    # IMPORTANT: JSON column via streaming must be sent as a JSON STRING
    r["raw_data"] = json.dumps(orig_row, separators=(",", ":"))

    # HARD FILTER: drop anything not in schema (kills averageScrollDepth, totalBotSessionCount, etc.)
...
def fetch_clarity_pageviews(api_key, clarity_project_id, days=45):
    # Clarity Data Export API only supports last 1-3 days, and 10 calls/day/project
...
def flatten_pageview(row):
    # For each entry in information[], create a row with metricName and all info fields as top-level keys
...
def insert_pageviews_into_bigquery(rows):
    client = bigquery.Client(project=PROJECT_ID)
    insert_ids = [
        f"{r.get('clarity_project_id','')}|{r.get('metricName','')}|{r.get('snapshot_date','')}"
        for r in rows
    ]
    errors = client.insert_rows_json(TABLE_ID, rows, row_ids=insert_ids)
    if errors:
        raise RuntimeError(f"BigQuery insert failed: {errors}")
    print(f"✅ Inserted {len(rows)} pageviews into BigQuery.")

def run_clarity_pipeline():
    print("🚀 Starting Clarity Pageviews Pipeline")
    api_key = fetch_secret("labessentials_prod_clarity_api_key")
    clarity_project_id = fetch_secret("labessentials_clarity_project_id")
    ...
    # flatten export to per-metric rows
    all_flat = []
    for row in raw_data:
    ...
    # insert with deterministic insertIds for dedupe
